...
At VitalSource, our answer to that question is a technology we call LearnKit. LearnKit is a set of APIs and SDKs that allows partners - be they publishers, institutions, edtech startups, whoever - to quickly and easily create new learning experiences wrapped around any content delivered in through platform.

What does that mean? That means that you can create a next-generation EPUB 3 once, get it into our system and use the content in that book in any number of ways, without rebuilding all of the complex backend architecture that you need to be a best-in-class learning platform. No need to build a content licensing model, note and highlight cross-device syncing, enterprise-level content DRM, an analytics pipeline - instead, focus on the user experience. Let's take a quick tour through a few examples.

The first example is actually...this presentation! The screens I've been showing so far are part of an EPUB 3 book, which is ingested into our platform and licensed to me - if I logged in as a different user I wouldn't have seen those slides. I used the LearnKit Web API to render out those EPUB 3 content documents right into the flow of an HTML presentation. Fun stuff. To contrast that, we could look at the same presentation within Bookshelf, our etextbook platform.

Here you can see the same chapter on "In C" within the Bookshelf UI and UX. We've been building on top of LearnKit for over a year now - every page viewed in the last year, over 2 billion, was viewed using LearnKit.

Let's take a quick look under the hood. I've labeled a few key areas of Bookshelf with the function that you'd call to pull back data. Retrieve the table of contents, get the user's highlights, render page 5 - these are the kinds of tools LearnKit exposes to the browser.

Of course you can use these to build something that looks completely different from Bookshelf, maybe to build towards a different market or vertical quickly - for example, here is a kid-friendly reader built using LearnKit that looks completely different. We built this in a partnership with Scholastic, and while you can see it uses those same functions, and under the hood 90% of the platform is the same, the reading experience much more appropriate for Scholastic's grade school audience.

Of course you don't need to build an entire ereader. We're currently working with a few different partners who want to display just a small chunk of content in another context - for example an adaptive courseware platform, that allows the learner to get a question wrong and right there within that context see the appropriate chunk of content to set them back on the right track.

The key here is to get the student the right content or interactive exercise - not to build a lot of bells and whistles and features around it, or overwhelm them with an entire textbook. That kind of simple content snippet experience is easy to build with LearnKit - here's an example "snippet viewer".

You can see we can set the start and stop page, or tweak it, and just the perfect chunk of content will render out. Build the content once - it may be just a portion of text, it may be our interactive collaborative "In C" music player, or anything in between - and reuse that chunk of content as often as you need in any context.
...
This is what will enable us to break out from a world of paper under glass. What can your content be? Where can that content go? Let's find out. Let's think outside the textbook.
